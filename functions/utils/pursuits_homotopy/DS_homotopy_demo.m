...
e = randn(M,1)*sigma;
y = A*x+e;
thresh = 1e-4; %sqrt(2*log(N))*sigma;
...
while ~done
    ...
    if constraint_plots
        ...
        if chk_x == 0
            subplot(211);
            plot(new_lambda, Primal_constrk(new_lambda),'or','MarkerSize',16);
            %legend('Old constraint', 'New constraint', 'Element change','Location','NorthEast');
        else
            subplot(212)
            plot(out_x, (ak(out_x)),'*k','MarkerSize',16);
            hold on;
            %legend('Old constraint', 'New constraint', 'Element
            %change','Location','NorthEast');
        end

        if chk_lambda == 0
            plot(new_x, Dual_constrk(new_x),'or','MarkerSize',16);
        else
            subplot(211);
            plot(out_lambda, (Primal_constrk(out_lambda)),'*k','MarkerSize',16);
            %legend('Old constraint', 'New constraint', 'Element change','Location','NorthEast');
        end
    end
%     [iter epsilon delta theta length(gamma_xk)]
end
time_e = clock;
etime(time_e,time_s);
